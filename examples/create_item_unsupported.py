...
def do_signin():
    ...
    try:
        op = OP(vault="Test Data", use_existing_session=True, password_prompt=False, op_path=op_path)
...
def main():
    ...
    if not op.supports_item_creation():
        print(f"pyonepassword does not support item creation with op version {op._cli_version}")
