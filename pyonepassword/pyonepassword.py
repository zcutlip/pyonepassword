...
class OP:
    ...
    def __init__(self, op_path=OP_PATH, signin_address=None, email_address=None, secret_key=None, logger=None):
        ...
        self.logger = logger
        initial_signin_args = [signin_address, email_address, secret_key]
        initial_signin = (None not in initial_signin_args)

        if initial_signin:
            self.token = self._do_initial_signin(*initial_signin_args)
        else:
            self.token = self._do_normal_signin()

    def _do_normal_signin(self):
        self.logger.info("Doing normal (non-initial) 1Password sign-in")
        signin_argv = [self.op_path, "signin", "--output=raw"]
        print("")
        token = self._run_signin(signin_argv).rstrip()
        return token

    def _do_initial_signin(self, signin_address, email_address, secret_key):
    ...
    def _run_signin(self, argv):
        return self._run(argv, OPSigninException, capture_stdout=True, input_string=None)
